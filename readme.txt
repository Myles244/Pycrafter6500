...
pycrafter6500.savesequence(images,exposures,trigger in,dark time,trigger out, repetitions, name)

Encodes a sequence and saves it in a name.npz file so that they can be easily sent to the controller later using controller.loadsequence(name+'.npz').
Saving the encoded sequence in a file avoids needlessly repeating the same calculations significantly speeding up defining large sequences.



controller.loadsequence(name,exposures=None,trigger in=None,dark time=None,trigger out=None, repetitions=None)

Defines a sequence for pattern on the fly mode from a npz file generated by the pycrafter6500.savesequence() function.
If any of the optional arguments are defined, they will be used in the sequence instead of the value saved with the file.